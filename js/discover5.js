// js/discover5.js - Dark themed movies

const darkThemedMovies = LEVENY_MOVIES.filter(m => m.discover === 5).map(m => ({
    title: m.title,
    img: m.poster,
    link: m.href.replace('../', '')
}));
...
function updatePageTitle() {
    ...
    if (pageTitle) pageTitle.textContent = 'DARK THEMED MOVIES';
}

function initializePagination() {
    if (darkThemedMovies.length === 0) { showEmptyState(); return; }
    displayMovies();
    generatePageNumbers();
    updatePagination();
}
...
function showEmptyState() {
    moviesGrid.innerHTML = '<div class="empty-state"><i class="fas fa-film"></i><h3>No Dark Themed Movies Found</h3><p>No Dark themed movies are currently available.<br>Check back later or browse other movie themes.</p></div>';
...
function displayMovies() {
    ...
    const pageMovies = darkThemedMovies.slice((currentPage - 1) * moviesPerPage, currentPage * moviesPerPage);
    if (pageMovies.length === 0) { showEmptyState(); return; }
    pageMovies.forEach(movie => {
        const a = document.createElement('a');
        a.href = movie.link;
        a.className = 'grid-movie-item';
        a.innerHTML = '<div class="movie-image-container"><img src="' + movie.img + '" alt="' + movie.title + '" loading="lazy"><div class="image-overlay"></div></div><div class="movie-title"><h4>' + movie.title + '</h4><p>Dark Theme</p></div>';
        moviesGrid.appendChild(a);
    });
}

function getTotalPages() { return Math.max(1, Math.ceil(darkThemedMovies.length / moviesPerPage)); }
